...
def Export ():
    ...
    if len(str(ui.allpay.text())) != 0 and len(str(ui.EveryPay.text())) != 0 and len(str(ui.overpay.text())) != 0:
        ...
        book1.save('Платёж по месячно.xlsx')
        book1.close()
        book = openpyxl.open('Платёж по месячно.xlsx')
        ...
        print (sheet['A2'])
        book.save('Платёж по месячно.xlsx')
...
def visotaSlaider ():
    if ui.Srok.text() == '' :
        ui.horizontalSlider_srok.setSliderPosition(0)
    else:
        s=ui.Srok.text()
        s = s.replace(' ', '')
        ui.horizontalSlider_srok.setSliderPosition(float(s)/100000)
...
def storona1Slaider ():
    if ui.summa.text() == '' :
        ...
        s = ui.summa.text()
        s = s.replace(' ', '')
        ui.horizontalSlider_summa.setSliderPosition(float(s)/100000)

def storona2Slaider ():
    if ui.procentStav.text() == '' :
        ui.horizontalSlider_stav.setSliderPosition(0)
    else:
        s = ui.procentStav.text()
        s = s.replace(' ', '')
        ui.horizontalSlider_stav.setSliderPosition(float(s)/100000)

ui.summa.textEdited.connect(storona1Slaider)
ui.Srok.textEdited.connect(visotaSlaider)
ui.procentStav.textEdited.connect(storona2Slaider)
'''
#выбр месяцов или лет
ui.comboBox.currentIndexChanged.connect(mounthORage)
ui.comboBox.currentIndexChanged.connect(mounthORage)
        #connect
ui.grafik_3.clicked.connect(Export)
